...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function loadProject(projectId) {
        ...
        // Check if data already exists - with safe handling
        let hasExistingData = false;
        if (typeof PdfModule.getPdfPageData === 'function') {
            hasExistingData = Object.keys(PdfModule.getPdfPageData()).length > 0;
        }
        ...
        // Reset stored data when starting a new analysis - with safe handling
        if (typeof PdfModule.resetPdfState === 'function') {
        ...
        if (typeof window.resetPdfState === 'function') {
        window.resetPdfState();
        }
        
        // Add a small timeout to ensure the loader is rendered
        // before the potentially resource-intensive API call starts
        setTimeout(() => {
            // Reset stored data when starting a new analysis
            PdfModule.resetPdfState();
            ...
            .then(data => {
                ...
                // Ensure loader is hidden after everything is done
                loader.style.display = 'none';
            })
            .catch(error => {
        ...
        }, 50); // Small timeout to ensure UI updates before fetch starts
    });
